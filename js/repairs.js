...
const Repairs = {
    ...
    async loadRepairs() {
        try {
            ...
            // Las reparaciones pueden tener branch_id, filtrar si existe
            let repairs = await DB.getAll('repairs', null, null, { 
                filterByBranch: !viewAllBranches, 
                branchIdField: 'branch_id' 
            }) || [];
            
            // Si no es admin y hay branch_id en las reparaciones, filtrar manualmente
            if (!viewAllBranches && currentBranchId) {
                const normalizedBranchId = String(currentBranchId);
                repairs = repairs.filter(r => {
                    // Si la reparación no tiene branch_id, incluirla (compatibilidad)
                    if (!r.branch_id) return true;
                    const repairBranchId = String(r.branch_id);
                    return repairBranchId === normalizedBranchId;
                });
            }
