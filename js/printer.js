...
// Soporta: EC Line 58110, GP-5830, y otras impresoras térmicas 58mm
...
const Printer = {
    ...
    commands: {
        ...
        // Comandos adicionales para GP-5830
    ...
    async connect(baudRate = null) {
        try {
            ...
            // Estrategia de conexión mejorada para GP-5830
            ...
            try {
                Utils.showNotification('Selecciona tu impresora GP-5830 en la lista de dispositivos', 'info');
                ...
                try {
                    ...
                    if (this.preferredFilters && this.preferredFilters.length > 0) {
                        try {
                            ...
                            throw new Error('No se pudo encontrar ningún dispositivo compatible. Asegúrate de que la GP-5830 esté conectada por USB y que el navegador tenga permisos para acceder a puertos seriales.');
                        }
                    } else {
                        throw new Error('No se pudo encontrar ningún dispositivo compatible. Asegúrate de que la GP-5830 esté conectada por USB y que el navegador tenga permisos para acceder a puertos seriales.');
                    }
    ...
    async writeText(text, useBold = true) {
        ...
        // Convertir texto a bytes usando codificación correcta para GP-5830
        const bytes = [];
    ...
    replaceSpecialChar(charCode) {
        // Mapeo completo de caracteres especiales para GP-5830
        const replacements = {
    ...
    async printTicket(sale) {
        ...
        try {
            ...
            try {
                ...
                console.warn('Error obteniendo configuración de impresora, usando valores por defecto:', e);
                settings = {
            ...
            if (date) await this.writeText(this.mkline('FECHA:', date) + '\r\n', true);
            if (time) await this.writeText(this.mkline('HORA:', time) + '\r\n', true);
    ...
    async printTicketFallback(sale) {
        ...
        try {
            ...
            try {
                ...
                console.warn('Error obteniendo configuración en fallback:', e);
            }

            const ticketHTML = this.buildTicketHTML(sale, items, payments, branch, seller, guide, agency, settings);
            
            // Usar iframe oculto para evitar abrir ventanas
            // Crear un nuevo iframe cada vez para evitar conflictos
            const iframe = document.createElement('iframe');
            iframe.id = 'print-frame-' + Date.now(); // ID único
            ...
            const printWhenReady = () => {
                ...
                try {
                    ...
                    Utils.showNotification('Ticket enviado a impresora', 'success');
            ...
            // NO usar onload porque puede dispararse múltiples veces
            const checkAndPrint = () => {
                if (printCalled) {
                    cleanup();
                    return;
...
<body>
    <div class="center">
        <h1>${businessName}</h1>
        <div style="font-size: 15pt !important; font-weight: 900 !important;">${branch?.name || 'Tienda'}</div>
        <div style="font-size: 13pt !important; font-weight: 900 !important;">Folio: ${sale.folio}</div>
        <div style="font-size: 13pt !important; font-weight: 900 !important;">${this.formatDate(sale.created_at)}</div>
        </div>
    <div class="line"></div>
    ...
    ${items.map(item => `
        ...
        ${item.discount > 0 ? `<div style="font-size: 12pt !important; font-weight: 900 !important;">Desc: ${item.discount}%</div>` : ''}
    `).join('')}
    <div class="line"></div>
