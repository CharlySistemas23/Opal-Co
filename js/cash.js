...
const Cash = {
    ...
    async loadCurrentSession() {
        try {
            ...
            const todaySessions = allSessions.filter(s => {
                ...
                return sessionDate === today && s.branch_id === branchId && s.status === 'abierta';
            });
    ...
    async calculateCurrentTotals() {
        ...
        const sessionSales = sales.filter(s => {
            ...
            return saleDate === sessionDate && s.branch_id === branchId && s.status === 'completada';
        });
    ...
    async generateDailyProfit() {
        try {
            const branchId = this.currentSession?.branch_id || localStorage.getItem('current_branch_id');
            const today = Utils.formatDate(new Date(), 'YYYY-MM-DD');
