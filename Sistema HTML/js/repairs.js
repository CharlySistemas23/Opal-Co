...
const Repairs = {
    ...
    setupUI() {
        ...
        content.innerHTML = `
            <div class="module-actions" style="margin-bottom: var(--spacing-md); display: flex; gap: var(--spacing-sm); flex-wrap: wrap; width: 100%; box-sizing: border-box;">
                <button class="btn-primary" id="repair-add-btn" style="white-space: nowrap;">Nueva Reparación</button>
                <button class="btn-secondary" id="repair-export-btn" style="white-space: nowrap;">Exportar</button>
            </div>
            <div class="inventory-filters" style="width: 100%; max-width: 100%; box-sizing: border-box; margin-bottom: var(--spacing-md);">
    ...
    async loadRepairs() {
        try {
            let repairs = await DB.getAll('repairs');
    ...
    async displayRepairStats(repairs) {
        ...
        statsContainer.innerHTML = `
            ...
            ${monthlyData.length > 0 ? `
                <div class="dashboard-section" style="width: 100%; max-width: 100%; box-sizing: border-box; margin-bottom: var(--spacing-lg);">
                    <h3 style="font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: var(--spacing-sm);">Tendencia de Reparaciones (Últimos 6 Meses)</h3>
    ...
    async showAddForm(repairId = null) {
        ...
        const customers = await DB.getAll('customers');
        const items = await DB.getAll('inventory_items').filter(i => i.status !== 'vendida');
