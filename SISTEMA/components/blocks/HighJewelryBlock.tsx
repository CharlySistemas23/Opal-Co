...
import type { GridProduct } from "./BlockRenderer";

const placeholderPieces: GridProduct[] = [
...
interface HighJewelryBlockProps {
  data: Record<string, unknown>;
  pieces: GridProduct[];
}

export function HighJewelryBlock({ data, pieces }: HighJewelryBlockProps) {
  const limit = (data.limit as number) ?? 4;
  const displayPieces = (pieces.length > 0 ? pieces : placeholderPieces).slice(0, limit);
            ...
            >
              <Link href={`/products/${piece.handle}`} className="group block">
                <div className="relative aspect-[4/5] overflow-hidden mb-8 min-h-0 bg-charcoal/50">
                  <img
                    src={piece.image || "https://images.unsplash.com/photo-1605100804763-247f67b3557e?w=1200&q=80"}
                    alt={piece.title}
