...
function parseFilterOptions(raw: string | undefined): FilterOption[] {
  if (!raw || typeof raw !== "string") return defaultFilterOptions;
  try {
    const parsed = JSON.parse(raw) as FilterOption[];
    if (Array.isArray(parsed) && parsed.length > 0) {
      return parsed.filter((o) => o.value && o.label);
    }
  } catch {
    /* ignore */
  }
...
export default async function CollectionPage({ params }: CollectionPageProps) {
  ...
  const [collection, siteText] = await Promise.all([
    getCollectionForPublic(slug),
    getSiteTextMap(),
  ]);
  const filterOptions = parseFilterOptions(siteText.filter_options);
  const filterLayout = siteText.collection_filter_layout === "sidebar" ? "sidebar" : "horizontal";
  const title = collection?.title ?? slug.replace(/-/g, " ");
