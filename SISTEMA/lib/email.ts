...
export async function sendCustomerOtpEmail(
  ...
  const subject = "Your OPAL & CO login code";
  const text = `Your one-time login code is: ${code}. It expires in 10 minutes.`;
  ...
  if (isResendConfigured()) {
    try {
      ...
      console.error("[sendCustomerOtpEmail] Resend error:", e);
      return { ok: false, error: "EMAIL_FAILED" };
    }
  }
  ...
  if (isPostmarkConfigured()) {
    try {
      ...
      console.error("[sendCustomerOtpEmail] Postmark error:", e);
      return { ok: false, error: "EMAIL_FAILED" };
    }
  }

  console.log("[sendCustomerOtpEmail] No email configured. OTP for", to, ":", code);
  return { ok: true };
}
