// Tourist Report Module - Gestión Avanzada de Reportes de Turistas
...
const TouristReport = {
    ...
    async init() {
        if (this.initialized) {
            const activeTab = document.querySelector('#tourist-tabs .tab-btn.active')?.dataset.tab || 'report';
    ...
    setupUI() {
        ...
        content.innerHTML = `
            <div id="tourist-tabs" class="tabs-container" style="margin-bottom: var(--spacing-lg);">
                <button class="tab-btn active" data-tab="report"><i class="fas fa-file-alt"></i> Reporte</button>
                ...
                <button class="tab-btn" data-tab="reconciliation"><i class="fas fa-balance-scale"></i> Conciliación</button>
            </div>
            <div id="tourist-content"></div>
        `;

        // Event listeners para tabs
        document.querySelectorAll('#tourist-tabs .tab-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const clickedBtn = e.target.closest('.tab-btn');
                if (!clickedBtn) return;
                
                document.querySelectorAll('#tourist-tabs .tab-btn').forEach(b => b.classList.remove('active'));
    ...
    async loadTab(tab) {
        const content = document.getElementById('tourist-content');
        if (!content) return;
        ...
        try {
            ...
            switch(tab) {
                case 'report':
                    await this.loadOrCreateReport();
                    content.innerHTML = await this.getReportTab();
                ...
                case 'history':
                    ...
                    break;
                case 'analytics':
                    content.innerHTML = await this.getAnalyticsTab();
                    await this.loadAnalytics();
                    break;
                case 'reconciliation':
                    content.innerHTML = await this.getReconciliationTab();
    ...
    async getReportTab() {
        return '<div id="tourist-report-container"></div>';
    },
    ...
    async loadOrCreateReport() {
        const branchId = localStorage.getItem('current_branch_id');
        const today = Utils.formatDate(new Date(), 'YYYY-MM-DD');

        try {
            const reports = await DB.query('tourist_reports', 'date', today);
            const branchReport = reports.find(r => r.branch_id === branchId);

            if (branchReport) {
                this.currentReport = branchReport;
                this.displayReport();
            } else {
    ...
    async createNewReport() {
        ...
        // Get exchange rates from settings (solo USD y CAD)
        const exchangeUsd = await DB.get('settings', 'exchange_rate_usd');
        const exchangeCad = await DB.get('settings', 'exchange_rate_cad');
        
        const exchangeRateUsd = parseFloat(exchangeUsd?.value || localStorage.getItem('daily_exchange_rate') || '20.00');
        ...
        const report = {
            ...
            exchange_rate_cad: exchangeRateCad,
            status: 'abierto',
            observations: '',
            total_cash_usd: 0,
            total_cash_mxn: 0,
        ...
        await DB.add('tourist_reports', report);
        this.currentReport = report;
        this.displayReport();
    },

    async displayReport() {
        const container = document.getElementById('tourist-report-container');
        if (!container) return;
        ...
        const exchangeRateUsd = parseFloat(exchangeUsd?.value || this.currentReport?.exchange_rate_usd || localStorage.getItem('daily_exchange_rate') || '20.00');
        const exchangeRateCad = parseFloat(exchangeCad?.value || this.currentReport?.exchange_rate_cad || '15.00');
        ...
        container.innerHTML = `
            <div class="tourist-report-container" style="max-width: 100%;">
                <!-- HEADER SUPERIOR -->
                <div class="report-header" style="background: var(--color-bg-secondary); padding: var(--spacing-md); border-radius: var(--radius-md); margin-bottom: var(--spacing-lg); border: 1px solid var(--color-border-light); width: 100%; box-sizing: border-box;">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-md); align-items: end; width: 100%;">
                        <div class="form-group" style="margin-bottom: 0; min-width: 0;">
                            ...
                            <input type="date" id="tr-date" class="form-input" value="${this.currentReport?.date || Utils.formatDate(new Date(), 'YYYY-MM-DD')}" style="font-size: 13px; width: 100%;">
                        </div>
                        <div class="form-group" style="margin-bottom: 0; min-width: 0;">
                            <label style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: var(--spacing-xs);">TIPO DE CAMBIO</label>
                            <input type="number" id="tr-exchange-rate" class="form-input" step="0.0001" value="${exchangeRateUsd}" style="font-size: 13px; width: 100%;" readonly>
                            <small style="font-size: 10px; color: var(--color-text-secondary); display: block; margin-top: var(--spacing-xs);">USD: ${exchangeRateUsd.toFixed(4)} | CAD: ${exchangeRateCad.toFixed(4)}</small>
                        </div>
                        <div class="form-group" style="margin-bottom: 0; min-width: 0;">
                            <label style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: var(--spacing-xs);">FECHA</label>
                            <input type="text" class="form-input" value="${Utils.formatDate(new Date(this.currentReport?.date || new Date()), 'DD/MM/YYYY')}" readonly style="font-size: 13px; width: 100%;">
                        </div>
                        <div class="form-group" style="margin-bottom: 0; min-width: 0;">
                            ...
                            <input type="text" id="tr-branch" class="form-input" value="${branchName}" readonly style="font-size: 13px; width: 100%;">
                        </div>
                    </div>
                </div>
                ...
                <div class="arrivals-section" style="background: var(--color-bg-card); padding: var(--spacing-lg); border-radius: var(--radius-md); margin-bottom: var(--spacing-lg); border: 2px solid var(--color-primary); width: 100%; box-sizing: border-box;">
                    <div style="display: flex; align-items: center; gap: var(--spacing-md); margin-bottom: var(--spacing-md); flex-wrap: wrap;">
                        <h3 style="margin: 0; font-family: 'Inter', sans-serif; font-size: 14px; font-weight: 700; color: var(--color-text); text-transform: uppercase; letter-spacing: 0.5px; white-space: nowrap;">
                            <i class="fas fa-plane-arrival"></i> LLEGADAS / PASAJEROS POR AGENCIA (DEL DÍA)
                        </h3>
                    ...
                    <div id="arrivals-table-container" style="width: 100%; overflow-x: auto;">
                        ${await this.getArrivalsTableHTML(branchId, this.currentReport?.date || Utils.formatDate(new Date(), 'YYYY-MM-DD'))}
                    </div>
                ...
                <!-- CUADRO MAESTRO - ENTRADA RÁPIDA -->
                <div class="master-form" style="background: var(--color-bg-card); padding: var(--spacing-lg); border-radius: var(--radius-md); margin-bottom: var(--spacing-lg); border: 2px solid var(--color-accent); box-shadow: var(--shadow-md); width: 100%; box-sizing: border-box;">
                    <div style="display: flex; align-items: center; gap: var(--spacing-md); margin-bottom: var(--spacing-sm); flex-wrap: wrap;">
                        <h3 style="margin: 0; font-family: 'Inter', sans-serif; font-size: 14px; font-weight: 700; color: var(--color-text); text-transform: uppercase; letter-spacing: 0.5px; white-space: nowrap;">ENTRADA RÁPIDA</h3>
                        <div style="flex: 1; height: 1px; background: var(--color-border); min-width: 100px;"></div>
                    </div>
                    <div class="form-group" style="margin-bottom: var(--spacing-sm); width: 100%;">
                        <label style="font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; display: block;">Ingresa todos los datos separados por comas (presiona Enter para agregar)</label>
                        <input 
                            type="text" 
                            id="tr-master-input" 
                            class="form-input" 
                            placeholder="Ej: carlos, sebastian, travelex, 12, anillo de diamantes, 120 mxn, 12 gramos"
                            style="font-size: 13px; padding: var(--spacing-sm); font-weight: 400; width: 100%; box-sizing: border-box;"
                            autocomplete="off"
                        >
                    </div>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-sm); padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); font-size: 11px; color: var(--color-text-secondary); width: 100%;">
                        <div><strong>Formato:</strong> Vendedor, Guía, Agencia, Cantidad, Productos, Precio [Moneda], Peso (g)</div>
                        <div><strong>Ejemplo:</strong> carlos, sebastian, travelex, 12, anillo de diamantes, 120 mxn, 12</div>
                        <div><strong>Monedas:</strong> mxn, usd, eur, cad (al final del precio)</div>
                    </div>
                </div>

                <!-- TABLA PRINCIPAL -->
                <div class="report-table-section" style="margin-bottom: var(--spacing-lg); width: 100%;">
                    <div style="overflow-x: auto; border: 1px solid var(--color-border-light); border-radius: var(--radius-md); width: 100%; -webkit-overflow-scrolling: touch;">
                        <table class="cart-table" style="min-width: 1200px; font-size: 12px; width: 100%; table-layout: auto;">
                            <thead>
                                <tr style="background: var(--color-bg-secondary);">
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px;">VENDEDOR</th>
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px;">GUÍA</th>
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px;">AGENCIA</th>
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px;">CANT.</th>
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px; min-width: 180px;">PRODUCTOS</th>
                                    <th colspan="4" style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px; text-align: center; border-left: 1px solid var(--color-border);">CASH</th>
                                    <th colspan="2" style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px; text-align: center; border-left: 1px solid var(--color-border);">TPV</th>
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px; border-left: 1px solid var(--color-border);">TOTAL</th>
                                    <th style="padding: 8px 6px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px;">ACCIONES</th>
                                </tr>
                                <tr style="background: var(--color-bg-secondary);">
                                    <th></th>
                                    <th></th>
                                    ...
                                    <th></th>
                                    <th></th>
                                    <th style="padding: 6px 8px; font-size: 9px; font-weight: 600;">CAD</th>
                                    <th style="padding: 6px 8px; font-size: 9px; font-weight: 600;">USD</th>
                                    <th style="padding: 6px 8px; font-size: 9px; font-weight: 600;">PESOS</th>
                                    <th style="padding: 6px 8px; font-size: 9px; font-weight: 600;">VISA-MC</th>
                                    <th style="padding: 6px 8px; font-size: 9px; font-weight: 600;">AMEX</th>
                                    <th></th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody id="tr-lines-tbody">
                                <tr>
                                    <td colspan="13" style="text-align: center; padding: 40px; color: var(--color-text-secondary);">No hay renglones. Agrega uno desde el formulario superior.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <!-- SECCIÓN INFERIOR -->
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: var(--spacing-lg); width: 100%;">
                    <!-- TARJETAS BANCARIAS Y OBSERVACIONES -->
                    <div style="background: var(--color-bg-card); padding: var(--spacing-lg); border-radius: var(--radius-lg); border: 1px solid var(--color-border-light); min-width: 0;">
                        <h4 style="margin-bottom: var(--spacing-md); font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">TARJETAS BANCARIAS</h4>
                        <div class="form-group">
                            <label style="font-size: 11px; font-weight: 600;">TARJ. AMEX</label>
                            <input type="number" id="tr-total-amex" class="form-input" step="0.01" value="0" style="font-size: 13px; width: 100%;">
                        </div>
                        <div class="form-group">
                            <label style="font-size: 11px; font-weight: 600;">TARJ. DISCOVERY</label>
                            <input type="number" id="tr-total-discovery" class="form-input" step="0.01" value="0" style="font-size: 13px; width: 100%;">
                        </div>
                        <div class="form-group" style="margin-top: var(--spacing-md);">
                            <label style="font-size: 11px; font-weight: 600;">OBSERVACIONES</label>
                            <textarea id="tr-observations" class="form-textarea" rows="4" style="font-size: 12px; min-height: 80px; width: 100%; resize: vertical;">${this.currentReport?.observations || ''}</textarea>
                        </div>
                    </div>

                    <!-- COMISIONES Y CASH TOTALS -->
                    <div style="background: var(--color-bg-card); padding: var(--spacing-lg); border-radius: var(--radius-lg); border: 1px solid var(--color-border-light); min-width: 0;">
                        <h4 style="margin-bottom: var(--spacing-md); font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">COMISIONES Y CASH</h4>
                        <div class="form-group">
                            <label style="font-size: 11px; font-weight: 600;">Comisiones Vendedores</label>
                            <input type="text" id="tr-comm-sellers" class="form-input" value="$0.00" readonly style="font-size: 13px; background: var(--color-bg-secondary); font-weight: 600; width: 100%;">
                        </div>
                        <div class="form-group">
                            <label style="font-size: 11px; font-weight: 600;">Comisiones Guías</label>
                            <input type="text" id="tr-comm-guides" class="form-input" value="$0.00" readonly style="font-size: 13px; background: var(--color-bg-secondary); font-weight: 600; width: 100%;">
                        </div>
                        <div class="form-group">
                            <label style="font-size: 11px; font-weight: 600;">TOTAL CASH USD</label>
                            <input type="text" id="tr-total-cash-usd" class="form-input" value="$0.00" readonly style="font-size: 13px; background: var(--color-bg-secondary); font-weight: 600; width: 100%;">
                        </div>
                        <div class="form-group">
                            <label style="font-size: 11px; font-weight: 600;">TOTAL CASH PESOS</label>
                            <input type="text" id="tr-total-cash-mxn" class="form-input" value="$0.00" readonly style="font-size: 13px; background: var(--color-bg-secondary); font-weight: 600; width: 100%;">
                        </div>
                    </div>

                    <!-- TOTALES -->
                    <div style="background: var(--color-bg-card); padding: var(--spacing-lg); border-radius: var(--radius-lg); border: 1px solid var(--color-border-light); min-width: 0;">
                        <h4 style="margin-bottom: var(--spacing-md); font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">TOTALES</h4>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-sm); margin-bottom: var(--spacing-sm);">
                            <div style="font-size: 12px; font-weight: 600; padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm);">SUBTOTAL</div>
                            <div style="font-size: 13px; font-weight: 700; padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); text-align: right;" id="tr-subtotal">$0.00</div>
                        </div>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-sm); margin-bottom: var(--spacing-sm);">
                            <div style="font-size: 12px; font-weight: 600; padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm);">ADICIONALES</div>
                            <input type="number" id="tr-additional" class="form-input" step="0.01" value="${this.currentReport?.additional || 0}" style="font-size: 13px; padding: var(--spacing-sm); width: 100%;">
                        </div>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-sm); margin-top: var(--spacing-md); padding-top: var(--spacing-md); border-top: 2px solid var(--color-border);">
                            <div style="font-size: 14px; font-weight: 700; padding: var(--spacing-sm); background: var(--gradient-accent); color: white; border-radius: var(--radius-sm);">TOTAL</div>
                            <div style="font-size: 16px; font-weight: 700; padding: var(--spacing-sm); background: var(--gradient-accent); color: white; border-radius: var(--radius-sm); text-align: right;" id="tr-total">$0.00</div>
                        </div>
                    </div>
                </div>

                <!-- ACCIONES -->
                <div style="margin-top: var(--spacing-lg); display: flex; gap: var(--spacing-sm); justify-content: flex-end; flex-wrap: wrap; width: 100%;">
                    <button class="btn-secondary" onclick="window.TouristReport.reconcile()">Conciliar vs POS</button>
                    <button class="btn-secondary" onclick="window.TouristReport.exportReport()">Exportar</button>
                    <button class="btn-primary" onclick="window.TouristReport.closeReport()">Cerrar Reporte</button>
                </div>
            </div>
        `;

        // Setup event listeners
        this.setupMasterFormListeners(exchangeRateUsd, exchangeRateCad);
    ...
    async parseAndAddLine(inputText, exchangeRateUsd, exchangeRateCad) {
        ...
        // Extract price and currency from priceText (e.g., "120 mxn" or "50 usd" or "30 cad")
        let price = 0;
        let currency = 'MXN';
        
        const priceMatch = priceText.match(/([\d.]+)\s*(mxn|usd|cad)?/i);
        if (priceMatch) {
            price = parseFloat(priceMatch[1]) || 0;
            const currencyMatch = priceMatch[2];
            if (currencyMatch) {
            ...
            tbody.innerHTML = lines.map((line, idx) => {
                ...
                return `
                    <tr class="tourist-line" data-line-id="${line.id}" style="border-bottom: 1px solid var(--color-border-light);">
                        <td style="padding: 10px 8px; font-size: 12px;">${seller?.name || '-'}</td>
                        <td style="padding: 10px 8px; font-size: 12px;">${guide?.name || '-'}</td>
                        <td style="padding: 10px 8px; font-size: 12px;">${agency?.name || '-'}</td>
                        <td style="padding: 10px 8px; font-size: 12px; text-align: center;">${line.quantity || 0}</td>
                        <td style="padding: 10px 8px; font-size: 12px;">${line.products || ''}</td>
        ...
        const body = `
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-md); width: 100%; max-width: 100%; box-sizing: border-box;">
                ...
                <div class="form-group" style="min-width: 0; grid-column: 1 / -1;">
                    ...
                    <input type="text" id="line-products" class="form-input" value="${line.products || ''}" style="width: 100%;">
                </div>
                <div class="form-group" style="min-width: 0;">
    ...
    async updateArrival(agencyId, unitType) {
        const branchId = this.currentReport?.branch_id || localStorage.getItem('current_branch_id');
        const date = this.currentReport?.date || Utils.formatDate(new Date(), 'YYYY-MM-DD');
        ...
        // Para DISCOVERY en tienda 1 o 2, fijar city_tour
        const branch = await DB.get('catalog_branches', branchId);
        const branchName = branch?.name || '';
    ...
    async updateArrivalsTotals() {
        ...
        // Actualizar métricas del reporte (Ticket Promedio y % Cierre)
        await this.calculateTotals();
    }
};
