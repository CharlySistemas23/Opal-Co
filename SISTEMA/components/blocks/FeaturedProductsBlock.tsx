...
const placeholderProducts: GridProduct[] = [
...
export function FeaturedProductsBlock({ data, products }: FeaturedProductsBlockProps) {
  const limit = (data.limit as number) ?? 4;
  const items = products.length > 0 ? products.slice(0, limit) : placeholderProducts.slice(0, limit);
