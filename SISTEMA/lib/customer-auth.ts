...
const CUSTOMER_OTP_PEPPER =
  safeEnv("CUSTOMER_OTP_PEPPER") || "dev-customer-otp-pepper";
...
export function hashCustomerToken(token: string): string {
  return createHmac("sha256", CUSTOMER_OTP_PEPPER)
    .update(token)
    .digest("hex");
}

export function verifyCustomerOtpHash(plain: string, hash: string): boolean {
  const computed = hashCustomerToken(plain);
