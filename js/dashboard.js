...
const Dashboard = {
    ...
    async loadDashboard(showAllBranches = false) {
        try {
            ...
            const viewAllBranches = showAllBranches && canViewAllBranches;
            ...
            try {
                ...
                // Calcular costo de mercancía vendida (COGS) del día si no está en el reporte
                let merchandiseCost = todayProfit?.merchandise_cost || 0;
                if (!merchandiseCost && todaySales.length > 0) {
                    const saleItems = await DB.getAll('sale_items') || [];
                    merchandiseCost = 0;
                    for (const sale of todaySales) {
                        const items = saleItems.filter(si => si.sale_id === sale.id);
                        for (const item of items) {
                            const itemCost = item.cost || 0; // Costo de adquisición
                ...
                if (todayProfit) {
                    dailyProfit = {
                        ...
                        operating_costs: (todayProfit.fixed_costs_daily || 0) + (todayProfit.variable_costs_daily || 0) || todayProfit.operating_costs || 0,
                        commissions: (todayProfit.commissions_sellers_total || 0) + (todayProfit.commissions_guides_total || 0) || todayProfit.commissions || 0,
                        bank_commissions: todayProfit.bank_commissions || 0, // Comisiones bancarias
                        gross_profit: todayProfit.gross_profit || ((todayProfit.revenue_sales_total || todayProfit.revenue || 0) - merchandiseCost - (todayProfit.arrivals_total || todayProfit.arrival_costs || 0) - ((todayProfit.fixed_costs_daily || 0) + (todayProfit.variable_costs_daily || 0) || todayProfit.operating_costs || 0)),
                    ...
                    // Si no hay reporte pero hay ventas, calcular en tiempo real
                    const saleItems = await DB.getAll('sale_items') || [];
                    ...
                    const operatingCostsRealTime = 0; // Se calcularía desde cost_entries si es necesario
            ...
            // Costos del mes (filtrados por sucursal si no es vista consolidada)
