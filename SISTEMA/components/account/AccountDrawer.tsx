            ...
            <div className="flex-1 overflow-y-auto p-8">
              {isLoading ? (
                <Text variant="body" muted>
                  {t("account_drawer_loading", "Loading...")}
                </Text>
              ) : customer ? (
                <>
                  <h3 className="font-serif text-xl tracking-tight text-charcoal mb-2">
                    {t("account_drawer_welcome_prefix", "Welcome, ")}{customer.email}
                  </h3>
                  <Text variant="body" muted className="mb-8">
                    {t("account_drawer_logged_in_desc", "Manage your account and view your orders.")}
                  </Text>
                  <div className="space-y-4">
                    ...
                    >
                      {t("account_drawer_link_account", "Account")}
                    ...
                    <Link
                      ...
                      className="flex items-center justify-between font-sans text-sm uppercase tracking-[0.15em] text-charcoal hover:text-champagne transition-colors duration-fast py-4 border-b border-charcoal/10 group"
                    >
                      {t("account_drawer_link_orders", "Order history")}
                      <span className="group-hover:translate-x-1 transition-transform" aria-hidden>›</span>
                    </Link>
                    <div className="pt-4">
                      <LogoutButton />
                ...
                <>
                  <h3 className="font-serif text-xl tracking-tight text-charcoal mb-4">
                    {t("account_drawer_sign_in_prompt", "Sign in or create an account")}
                  </h3>
                  <Text variant="body" muted className="mb-8">
                    {t("account_drawer_sign_in_desc", "With an account, you can shop faster, view your order history, and access your bag or saved products from any device.")}
                  </Text>
                  <Link
                    href="/sign-in"
                    onClick={closeAccount}
                    className="flex items-center justify-between font-sans text-sm uppercase tracking-[0.15em] text-charcoal hover:text-champagne transition-colors duration-fast py-4 border-b border-charcoal/10 group"
                  >
                    {t("account_drawer_link_sign_in", "Iniciar sesión")}
                    <span className="group-hover:translate-x-1 transition-transform" aria-hidden>›</span>
                  </Link>
                  <Link
                    href="/register"
                    ...
                    className="flex items-center justify-between font-sans text-sm uppercase tracking-[0.15em] text-charcoal hover:text-champagne transition-colors duration-fast py-4 border-b border-charcoal/10 group"
                  >
                    {t("account_drawer_link_register", "Crear cuenta")}
                    <span className="group-hover:translate-x-1 transition-transform" aria-hidden>›</span>
                  </Link>
                </>
              )}
