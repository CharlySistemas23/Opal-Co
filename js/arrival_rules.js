...
const ArrivalRules = {
     ...
     * @param {string} unitType - Tipo de unidad (city_tour, sprinter, van, truck, null)
    ...
    async calculateArrivalFee(agencyId, branchId, passengers, unitType = null, dateYYYYMMDD = null) {
        try {
            ...
            let allRules = await DB.getAll('arrival_rate_rules', null, null, { 
            ...
            if (!viewAllBranches && currentBranchId) {
                ...
                allRules = allRules.filter(rule => {
                    if (!rule.branch_id) return true; // Incluir reglas sin branch_id
                    return String(rule.branch_id) === normalizedBranchId;
                });
            }
            // #region agent log
            ...
            const activeRules = allRules.filter(rule => {
                ...
                // Verificar tipo de unidad (si es null, aplica a todos)
                if (rule.unit_type && rule.unit_type !== unitType) return false;
