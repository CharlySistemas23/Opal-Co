...
const Costs = {
    ...
    async getFilteredCosts(options = {}) {
        ...
        // Si no hay branchId seleccionado y es admin, puede ver todas
        // Si hay branchId seleccionado, siempre filtrar por esa sucursal
        const viewAllBranches = !branchId && isAdmin;
        
        // Obtener costos con filtro de sucursal
        let costs = await DB.getAll('cost_entries', null, null, { 
        ...
        if (normalizedBranchId) {
            costs = costs.filter(c => {
                // Si el costo no tiene branch_id, incluirlo (compatibilidad)
                if (!c.branch_id) return true;
                const costBranchId = String(c.branch_id);
            ...
            <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light); margin-bottom: var(--spacing-md);">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: var(--spacing-sm);">
                    <h3 style="font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin: 0;">
                        <i class="fas fa-sync-alt"></i> Costos Recurrentes
                    </h3>
    ...
    async loadRecurring() {
        ...
        try {
            // Obtener sucursal actual y filtrar costos
            const currentBranchId = typeof BranchManager !== 'undefined' ? BranchManager.getCurrentBranchId() : null;
            const allCosts = await this.getFilteredCosts({ branchId: currentBranchId });
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: var(--spacing-md); margin-bottom: var(--spacing-md); width: 100%; max-width: 100%; box-sizing: border-box;">
                <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light); min-width: 0; width: 100%; box-sizing: border-box;">
                    ...
                    <div class="form-group">
                        <label>Mes</label>
                        <input type="month" id="budget-month" class="form-input" value="${Utils.formatDate(new Date(), 'YYYY-MM')}" style="width: 100%;">
                    </div>
                    <div class="form-group">
                        <label>Presupuesto Total</label>
                        <input type="number" id="budget-amount" class="form-input" step="0.01" placeholder="0.00" style="width: 100%;">
                    </div>
    ...
    async loadBudgetList() {
        ...
        // Los presupuestos se pueden almacenar en una nueva tabla o en settings
    ...
    async saveBudget() {
        ...
        // Aquí se guardaría en una tabla de presupuestos
        // Por ahora solo mostramos notificación
        Utils.showNotification('Presupuesto guardado (funcionalidad en desarrollo)', 'success');
        await this.loadBudget();
    },
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-md); width: 100%; box-sizing: border-box;">
                    ...
                    <div class="form-group" style="min-width: 0; grid-column: 1 / -1;">
                        ...
                        <select id="cost-branch" class="form-select" style="width: 100%;">
                            ...
                            ${branches.map(b => `<option value="${b.id}" ${cost?.branch_id === b.id ? 'selected' : ''}>${b.name}</option>`).join('')}
                        </select>
                    </div>
                    <div class="form-group" style="min-width: 0; grid-column: 1 / -1;">
                        <label>Notas</label>
    ...
    async saveCost(costId) {
        ...
        const cost = {
            ...
            branch_id: document.getElementById('cost-branch').value || null,
