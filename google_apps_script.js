...
function createIndexSheet(ss) {
  ...
  if (!indexSheet) {
    ...
    const sheetsInfo = [
      ...
      ['CATALOG_BRANCHES', 'Catálogo de sucursales'],
...
function addHeaders(sheet, sheetName) {
  ...
  switch (sheetName) {
    case SHEETS.SALES:
      ...
      break;
    case SHEETS.ITEMS:
      headers = ['ID', 'ID Venta', 'ID Producto', 'Cantidad', 'Precio', 'Costo', 'Descuento', 'Subtotal', 'Fecha Creación'];
      break;
    ...
    case SHEETS.CATALOG_BRANCHES:
      headers = ['ID', 'Nombre', 'Dirección', 'Teléfono', 'Activa', 'Fecha Creación', 'Fecha Actualización', 'Sincronizado'];
      break;
...
function adjustColumnWidths(sheet, sheetName, numColumns) {
  ...
  switch (sheetName) {
    case SHEETS.SALES:
      ...
      break;
    case SHEETS.ITEMS:
      widths = [80, 100, 100, 80, 100, 100, 100, 100, 150];
      break;
    case SHEETS.PAYMENTS:
...
function processSales(ss, records) {
  ...
  if (!sheet || !itemsSheet || !paymentsSheet) {
      continue;
  }
